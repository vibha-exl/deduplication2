...
transcript_path = r'/Workspace/Users/ananthagiri_abhiram@next.co.uk/iAudit_Trisha/chat_sample3_cleaned.xlsx'

# /Workspace/Users/ananthagiri_abhiram@next.co.uk/iAudit_Trisha/0512AllcallsforAccountAmendment.xlsx
result_path = r"ChatAccountAmendmentClosureFinalResult.xlsx"
...
if __name__=="__main__":
    ...
    try:
        ...
        # transcripts =transcripts.rename(columns={'id':'callid'})
        ...
        # Rename columns to match old voice pipeline
        ...
        spark_logger =  spark.sql(f"""
                ...
                WHERE id IN {tuple(audit_callids)}      
            """)
        ...
        df_logger_unique = (df_logger.sort_values(['ticket_id', 'priority'], ascending=[True, False]).drop_duplicates(subset = 'ticket_id', keep='first').drop(columns='priority'))
        # print(df_logger_unique.shape)
        df_logger_unique['ticket_id'] = df_logger_unique['ticket_id'].astype('Int64')

        data = pd.merge(df_transcripts, df_logger_unique, left_on='callid', right_on='ticket_id', how='left')
        # data['callid_agentid']= data['callid'] + "|" + data['AgentId']
        # print("data",data.columns)
        # grouped_transcript_all = data.groupby('callid').apply(lambda x: "\n".join(f"{row['convo']}" for _, row in x.iterrows())).reset_index(name = 'transcript_x')
        data_req = data.copy()
        # data_intermediate = data.groupby('callid', as_index = False).first()
        # data_req = pd.merge(grouped_transcript_all, data_intermediate, on='callid', how='left')
    ...
    # df_ai.toPandas().to_excel("AccountAmendemntIntermediateResults.xlsx")
        ...
        df_ai_pandas['Result: Account Amendment- Address Change ask place order'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change ask place order']=='Amber Error')&(df_ai_pandas['transcript'].str.contains('place an order', case=False, na=False)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change ask place order'] )

        df_ai_pandas['Result: Account Amendment- Address Change ask place order'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change ask place order']=='Amber Error')&(df_ai_pandas['transcript'].str.contains('place any new order', case=False, na=False)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change ask place order'] )

        df_ai_pandas['Result: Account Amendment- Address Change ask place order'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change ask place order']=='Amber Error')&(df_ai_pandas['transcript'].str.contains('place new order', case=False, na=False)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change ask place order'] )
        

        df_ai_pandas['Result: Account Amendment- Address Change ask place order'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change ask place order']=='Amber Error')&(df_ai_pandas['transcript'].str.contains('placed the order', case=False, na=False)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change ask place order'] )

        

        df_ai_pandas['Result: Account Amendment- Address Change ask place order'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change ask place order']=='Amber Error')&(df_ai_pandas['transcript'].str.contains('made the order', case=False, na=False)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change ask place order'] )

        df_ai_pandas['Result: Account Amendment- Address Change ask place order'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change ask place order']=='Amber Error')&(df_ai_pandas['transcript'].str.contains('placing orders', case=False, na=False)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change ask place order'] )

        df_ai_pandas['Result: Account Amendment- Address Change ask place order'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change ask place order']=='Amber Error')&(df_ai_pandas['transcript'].str.contains('placing an order', case=False, na=False)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change ask place order'] )

        


        df_ai_pandas['Result: Account Amendment- Address Change ask place order'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change ask place order']=='Amber Error')&(df_ai_pandas['transcript'].str.contains('placing any orders', case=False, na=False)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change ask place order'] )
        ...
        df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin']=='Red Error')&(df_ai_pandas['transcript'].str.contains('pin', case=False, na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] )

        df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin']=='Red Error')&(df_ai_pandas['transcript'].str.contains('4 digit pin', case=False, na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] )

        df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin']=='Red Error')&(df_ai_pandas['transcript'].str.contains('four digit pin', case=False,na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] )

        df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin']=='Red Error')&(df_ai_pandas['transcript'].str.contains('number ending', case=False, na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] )

        df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin']=='Red Error')&(df_ai_pandas['transcript'].str.contains('phone number', case=False, na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] )

        df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin']=='Red Error')&(df_ai_pandas['transcript'].str.contains('cell number', case=False, na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] )

        df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin']=='Red Error')&(df_ai_pandas['transcript'].str.contains('mobile number', case=False, na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] )

        df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin']=='Red Error')&(df_ai_pandas['transcript'].str.contains('contact number', case=False, na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change mobile access for pin'] )
        ...
        df_ai_pandas['Result: Account Amendment- Address Change pin fail security question'] = np.where((df_ai_pandas['Result: Account Amendment- Address Change pin fail security question']=='Red Error')&(df_ai_pandas['transcript'].str.contains('pin', case=False, na=False, regex=True)),'No Error', df_ai_pandas['Result: Account Amendment- Address Change pin fail security question'] )

        df_ai_pandas['pin present'] =df_ai_pandas['transcript'].str.contains('pin', case=False, na=False, regex=True)
        ...
        #df_ai_pandas.to_excel("AccountAmendemntIntermediateResultsAfterChange.xlsx")
        df_ai_pandas_filtered = df_ai_pandas[['callid','iAudit Result: Account Amendment and Closure','iAudit Score: Account Amendment and Closure','iAudit Remarks: Account Amendment and Closure', 'iAudit Confidence: Account Amendment and Closure']]
