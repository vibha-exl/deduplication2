...
result_path = r"x29InformationExplanationExplanationFinalResults.xlsx"
log = get_logger()
...
if __name__=="__main__":
    try:
        ...
        main_config = yaml.safe_load(open('/Workspace/root/ContactCentre/iAudit Processes/deployment2/main_config.yaml', 'r'))
        catalog_config = yaml.safe_load(open('/Workspace/root/ContactCentre/iAudit Processes/deployment2/catalog_config.yaml', 'r'))
...
###################################################################################
        ...
        # Path to adjustment file - update as needed
        ...
        data_req['payment_success_call'] = (
            ...
            &
            (data_req['transcript'].apply(contains_payment_success_keywords))
        )
        ...
        "Did the agent offer to update email for user account?",
        "If agent offered did the user agree to update email on the account?"
    ]
        ...
        df_ai_spark = (
            df_ai_spark.withColumn(
                "ai_response_payment_email", F.when(
                    ...
                    F.expr(
                        f"""ai_query('contact_centre_internal_batch', request => concat({prompt_email_confirmation_json}, transcript))"""
                    )
                )
            )
        )
        ...
        if len(refund_callids) > 0:
            ...
            df_refund_llm = (
                ...
                .withColumn(
                    ...
                    F.expr(f"""ai_query('contact_centre_internal_batch', request => concat({prompt_refund_email_confirmation_json}, transcript))""")
                )
                .toPandas()
            )
        ...
        # Return Charge - Email confirmation for refund
        # Catches "Return Charge" calls where a refund is mentioned in the transcript
        # but agent did not inform about email confirmation (not caught by GOGW check)
        df_ai_pandas['Result: Return Charge Refund Email'] = np.where(
            (
                (df_ai_pandas['Level 2'] == 'Return Charge')
                & (df_ai_pandas["Did the transcript mention refund of charges?"].str.lower() == 'yes')
                & (df_ai_pandas["Did the agent say email confirmation will be sent for refund of charges?"].str.lower() == 'no')
                & (df_ai_pandas['refund_found'] == 0)
            ),
            'Compliant with development',
            'No Error'
        )
