...
def grouping(df,logger):
    try:
        df = df[['callid', 'transcript', 'AgentId', 'createdate']]
...
def timescales_process(df_process, df1_process, logger):
    ...
    try:
        ...
        result_df = df_filtered.withColumn("result", expr(f"ai_query('databricks-gpt-oss-120b', request => concat({system_prompt_timescales_json}, transcript, '\nCall date:\n', call_date), responseFormat => '{response_format}')"))
...
def validate_refund_markdowns(dfspark):
    ...
    ai_query_expr =f"ai_query('databricks-gpt-oss-120b',request => concat('{validation_prompt}', '\n', transcript))"
...
def delivery_process(df_process, df1_process, logger):
    ...
    try:
        ...
        result_df = df_filtered.withColumn(
            "result",
            expr(f"ai_query('databricks-gpt-oss-120b', request => concat({system_prompt_delivery_json}, transcript), responseFormat => '{response_format_delivery}')")
        )
        #
        response_schema = StructType([
...
def payment_process(df_process, df1_process, logger):
    ...
    try:
        ...
        result_df = df_main.withColumn(
            "result",
            expr(f"ai_query('databricks-gpt-oss-120b', request => concat({system_prompt_payments_json}, transcript), responseFormat => '{response_format_payment}')")
        )
        response_schema = StructType([
...
def cancellation_process(df_process, df1_process, logger):
    ...
    try:
        ...
        result_df = df_main.withColumn(
            "result",
            expr(f"ai_query('databricks-gpt-oss-120b', request => concat({system_prompt_cancellation_json}, transcript), responseFormat => '{response_format_cancellation}')")
        )
...
if __name__=="__main__":
    ...
    try:
        ...
        # """)
        table = "contactcentre_prod.iaudit.filtered_input_data_iaudit_chat_v2"
        # table = catalog_config['output_table']['filtered_table']
        INPUT_FILE_PATH = spark.table(table).filter(f"call_date = '{formatted_date}'")
        ...
        # FILE_NAME="/Workspace/Users/pawan_kumar@next.co.uk/timsscale/data/chat_sample1_preprocessed.xlsx"
